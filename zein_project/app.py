    ...
    with admin_tab1:
        ...
        if st.button("Add Student", key="add_student_btn"):
            if not student_name:
                ...
                new_student = pd.DataFrame([{"student_name": student_name, "class_level": student_class}])
                students = pd.concat([students, new_student], ignore_index=True)
                save(students, STUDENTS_FILE)

                new_user = pd.DataFrame([{"username": student_name, "password": hash_password("1234"), "role": "student", "subject": ""}])
    ...
    with admin_tab2:
        ...
        if st.button("Add Teacher", key="add_teacher_btn"):
            if teacher_username in users.username.values:
                st.error(f"Teacher username already exists")
            else:
                new_user = pd.DataFrame([{"username": teacher_username, "password": hash_password("1234"), "role": "teacher", "subject": teacher_subject}])
                users = pd.concat([users, new_user], ignore_index=True)
                save(users, USERS_FILE)

                st.success(f"✅ Teacher '{teacher_username}' added successfully! Password: 1234")
                st.rerun()
    ...
    with tab1:
        ...
        if st.button("Save Marks", key="save_marks_btn"):
            try:
                ...
                results_updated = pd.concat([results_filtered, pd.DataFrame([{"student": selected_student, "class_level": selected_class, "term": selected_term, "total": total, "average": round(avg, 2), "grade": grade(avg), "rank": 0}])], ignore_index=True)
                ...
                st.success(f"✅ Marks saved! Average: {round(avg, 2)}, Grade: {grade(avg)}")
                st.rerun()
    ...
    with tab2:
        ...
        if st.button("Save Attendance", key="save_attendance_btn"):
            try:
                ...
                attendance_updated = pd.concat([attendance_filtered, pd.DataFrame([{"student": selected_student, "class_level": selected_class, "term": selected_term, "days_present": present, "total_days": total_days, "attendance_percent": percent}])], ignore_index=True)
